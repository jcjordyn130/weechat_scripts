...
# History:
# 2026-07-14, jcj <jcjordyn120@jordynsblog.org>:
#   v2.8: add custom time formatter and nick ignore list
...
sub highmon_config_init
{
	...
	# New options added by jcj
	if (!(weechat::config_is_set_plugin("ignore_nicks")))
	{
		weechat::config_set_plugin("ignore_nicks", "");
	}

	if (!(weechat::config_is_set_plugin("custom_time_format")))
	{
		weechat::config_set_plugin("custom_time_format", "");
	}
}
...
sub highmon_print
{
	...
	elsif (weechat::config_get_plugin("output") eq "bar")
	{
		...
		if ($cb_date)
		{
			$time = strftime($timestr, localtime($cb_date));
		}
		else
		{
			$time = strftime($timestr, localtime);
		}
